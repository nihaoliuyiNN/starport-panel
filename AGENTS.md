...
- `web/` — UI 源码（React 18 + Vite + Ant Design 5 + TanStack Query + xterm.js，pnpm）。约定：`src/api/` 是唯一的 HTTP 层（`types.ts` 与 Go JSON 一一对应，改后端结构必须同步）；页面用 antd 原生 `Table`，不引 pro-components；所有表格 `scroll={{ x: 'max-content' }}`；WebSocket 经 `wsUrl()` 带 `?token=`。UI 只消费公开 API，不得出现任何 UI 专用后端接口。
- `internal/panel/auth.go` — API 令牌中间件（Bearer / `?token=`），只保护 `/api/**`，静态资源放行。库内令牌 `store/tokens.go` 只存 sha256；静态令牌 `Config.APIToken`。单角色管理员模型，暂无细粒度权限——需要多租户 / RBAC 的是商业层的事，不要加进来。
