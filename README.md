...
> 当前状态：**Phase 0 —— 骨架与节点纳管**。装集群、应用管理、Web UI 见下方路线图。
...
生产节点安装：`scripts/install-starport-agent.sh`（systemd 常驻，见 `docs/build.md`）。
...
- [ ] Phase 1：节点持久化（SQLite）、装集群编排（首 master → join）、kubeconfig 接管、client-go 节点视图
- [ ] Phase 2：应用管理（部署 / 扩缩 / 暴露 / Helm）、Web 终端与日志（经 apiserver）、Web UI
